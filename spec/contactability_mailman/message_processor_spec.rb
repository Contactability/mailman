...
describe ContactabilityMailman::MessageProcessor do

  let(:message) { "To: mikel\r\nFrom: bob\r\nSubject: Hello!\r\n\r\nemail message\r\n" }
  let(:basic_email) { Mail.new message }
  let(:router) { mock('Message Router', :route => false) }
  let(:processor) { ContactabilityMailman::MessageProcessor.new(:router => router) }
  ...
  describe "#process" do
    ...
    it 'should log in info the new message received' do
      ContactabilityMailman.logger.should_receive(:info).with("Got new message from '#{basic_email.from.first}' with subject '#{basic_email.subject}'.")
      processor.process(basic_email)
    end

    it 'should receive email without from field' do
      ContactabilityMailman.logger.should_receive(:info).with("Got new message from 'unknown' with subject '#{basic_email.subject}'.")
  ...
  describe "#process_maildir_message" do
    ...
    it 'should log errors caused by processing the message, but not raise them so futher messages can be processed' do
      ...
      ContactabilityMailman.logger.should_receive(:error)
