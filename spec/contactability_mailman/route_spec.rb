...
describe ContactabilityMailman::Route do
...
class TestingCondition < ContactabilityMailman::Route::Condition
  def match(message)
    [{:testing => 'test'}, ['testing']] if @matcher.match(message)
  end
end

class TesterCondition < ContactabilityMailman::Route::Condition
  def match(message)
